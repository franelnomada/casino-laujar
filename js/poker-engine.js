...
class PokerRoom {
  ...
  addPlayer(id,name) {
    ...
    this.players.push({id,name:String(name||'Jugador').slice(0,12),chips:2000,hand:[],bet:0,total:0,folded:false,allIn:false,inHand:false,left:false,result:'',actedAt:null});
  ...
  removePlayer(id,now=Date.now()) {
    const p=this.find(id);if(!p)return {ok:true};
    p.left=true;
