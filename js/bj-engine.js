...
class BlackjackRoom {
  ...
  addPlayer(id, name, initChips) {
    ...
    // Respeta el saldo real del jugador, incluido el cero (sin recompra gratis al entrar).
    // El valor por defecto solo actúa como red de seguridad si no llega ningún valor.
    const chips = (initChips != null) ? Math.max(0, initChips) : 1000;
    this.players.push({
      id, name: (name || 'Jugador').slice(0, 12), chips, bet: 0,
  ...
  removePlayer(id) {
    ...
    if (i === -1) return { ok: true, chips: null };
